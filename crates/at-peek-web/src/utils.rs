...
use atproto_client::{Handle, LabelerClient, LabelCollection, PostClient, resolve_handle, resolve_did, create_session, Did, AtRecord};
...
pub async fn authenticate(handle: &str, password: &str) -> Result<String, String> {
    create_session(handle, password).await
        .map(|session| session.access_jwt)
        .map_err(|e| format!("Authentication failed: {}", e))
}

/// Fetch labels for a given subject (handle, DID, or AT-URI) from multiple sources
pub async fn fetch_labels(input: &str, auth_token: Option<String>) -> Result<LabelCollection, String> {
    ...
    let mut all_labels = match bsky_labeler.query_labels(&[subject.clone()]).await {
...
async fn fetch_likes(post_uri: &str) -> Result<(usize, Vec<UserInfo>), String> {
    ...
    if !response.status().is_success() {
        log::warn!("Failed to fetch likes for {}: {}", post_uri, response.status());
        return Ok((0, Vec::new()));
    }
    ...
    if let Some(likes) = json.get("likes").and_then(|l| l.as_array()) {
        for like in likes {
            if let (Some(did), Some(handle)) = (
                like.get("actor").and_then(|a| a.get("did")).and_then(|d| d.as_str()),
                like.get("actor").and_then(|a| a.get("handle")).and_then(|h| h.as_str()),
            ) {
                let display_name = like.get("actor")
...
async fn fetch_reposts(post_uri: &str) -> Result<(usize, Vec<UserInfo>), String> {
    ...
    if !response.status().is_success() {
        log::warn!("Failed to fetch reposts for {}: {}", post_uri, response.status());
        return Ok((0, Vec::new()));
    }
    ...
    let repost_count = json.get("repostedBy")
        ...
        match embed_type {
            Some("app.bsky.embed.images") => {
                ...
                if let Some(images) = embed.get("images").and_then(|i| i.as_array()) {
                    for img in images {
                        if let Some(cid) = img.get("image")
                            .and_then(|i| i.get("ref"))
                            .and_then(|r| r.get("$link"))
                            .and_then(|l| l.as_str()) {
                            let url = format!(
                                "{}/xrpc/com.atproto.sync.getBlob?did={}&cid={}",
                                pds_endpoint, did.as_str(), cid
                            );
                            image_urls.push(url);
                        }
                    }
            ...
            Some("app.bsky.embed.video") => {
                // Extract video CID
                if let Some(cid) = embed.get("video")
    ...
    F: FnMut(String),
{
    ...
    let posts = post_client.fetch_posts(&did, 1000).await
    ...
    if uris.contains(&takedown_uri) {
        ...
        log::warn!("✗ Known !takedown post NOT in fetched posts: {}", takedown_uri);
        log::warn!("This might mean the post was deleted, or not in the last {} posts", posts.len());
    }
    ...
    let bsky_labeler = if let Some(token) = auth_token.clone() {
        log::info!("Using authenticated labeler client (token: {}...)", &token[..20.min(token.len())]);
        LabelerClient::new_authenticated(token)
    } else {
    ...
    for (i, chunk) in uris.chunks(batch_size).enumerate() {
        progress_callback(format!(
            ...
            i + 1,
            (uris.len() + batch_size - 1) / batch_size
        ));
        ...
        match bsky_labeler.query_labels(chunk).await {
            Ok(collection) => {
                log::info!("Batch {} returned {} labels", i + 1, collection.labels.len());
    ...
    log::info!("Total labels found across all batches: {}", all_labels.len());
    
    progress_callback("Analyzing results...".to_string());
    
    // Calculate statistics
    let mut posts_with_labels_set: std::collections::HashSet<String> = std::collections::HashSet::new();
    ...
    for post in &posts {
        ...
        if !post_labels.is_empty() {
            ...
            let (like_count, likers) = fetch_likes(&post.uri).await
                .unwrap_or((0, Vec::new()));
            let (repost_count, reposters) = fetch_reposts(&post.uri).await
                .unwrap_or((0, Vec::new()));
            
            log::info!("Post {} has {} likes and {} reposts", post.uri, like_count, repost_count);
